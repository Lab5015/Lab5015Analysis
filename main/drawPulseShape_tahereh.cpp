...
int main(int argc, char** argv)
{
  ...
  //---------------
  // define outfile
  TFile* outFile = new TFile(Form("./plots/pulseShape_run%s_ch1_%d_ch2_%d.root",runs.c_str(), ch1, ch2),"RECREATE");
  ...
  for(auto mapIt : h1_tot_ch1)
    {  
      ...
      hPad = (TH1F*)( gPad->DrawFrame(-0.5,0.,63.5,1200*frequency*0.3) );
      //hPad = (TH1F*)( gPad->DrawFrame(-0.5,0.,63.5, g_N_totSel_ch1[Vov] -> GetY()[0]*2.5) );
  ...
  {  
    ...
    c = new TCanvas("c","c");
    hPad = (TH1F*)( gPad->DrawFrame(-0.5,0.,63.5,25.) );
  ...
  {  
    ...
    //hPad = (TH1F*)( gPad->DrawFrame(-2.,0.,20.,40.) );
    //hPad = (TH1F*)( gPad->DrawFrame(-2.,0.,20.,100.) );
    hPad = (TH1F*)( gPad->DrawFrame(-2.,0.,30.,65*dac_to_uA) );
  ...
  {
    mapIt.second -> Write(Form("g_ps_totSel_ch1_%d_Vov%.1f", ch1, mapIt.first));
  }
  for(auto mapIt : g_ps_totSel_ch2)
  {
    mapIt.second -> Write(Form("g_ps_totSel_ch2_%d_Vov%.1f", ch2, mapIt.first));
  }

  for (auto mapIt:  g_N_ch1)
  {
    mapIt.second -> Write(Form("g_N_ch1_%d_Vov%.1f", ch1, mapIt.first));
  } 

  for (auto mapIt:  g_N_ch2)
  {
    mapIt.second -> Write(Form("g_N_ch2_%d_Vov%.1f", ch2, mapIt.first));
  } 
