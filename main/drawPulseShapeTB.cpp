...
int main(int argc, char** argv)
{
  ...
  for(int entry = 0; entry < nEntries; ++entry)
	...
	//std::cout << "AAAAAAAAAAA " << index << " " << Vov << "  " << ith << "  " << iBar <<std::endl;
	...
	//if ( energyL[iBar] >= 940.) continue;
	//if ( energyR[iBar] >= 940.) continue;
  ...
  for(auto mapIt : h1_energyLR){
    ...
    if ( f_landau->GetParameter(1) > 0 ){                                                                                                                          
      xmin = f_landau->GetParameter(1) - 2 * std::abs(f_landau->GetParameter(2));                                                                           
  ...
  for(auto mapIt : h1_time1_totSel_chL)
    {
      ...
      //g_pulseShapeL[index2] -> SetPointError(g_pulseShapeL[index2]->GetN()-1,histo->GetMeanError(),0.);
  ...
  for(auto mapIt : h1_time1_totSel_chR)
    {
      ...
      //g_pulseShapeR[index2] -> SetPointError(g_pulseShapeR[index2]->GetN()-1,histo->GetMeanError(),0.);
  ...
  for(auto mapIt : g_N_L)
    {  
      ...
      //f_sigmoid_R[index2] -> SetParameters(g_N_R[index2]->GetPointY(0),12.,3.);
  ...
  {  
    ...
    fitFuncLowL -> SetRange(g_temp ->GetPointX(0), g_temp ->GetPointX(3));
