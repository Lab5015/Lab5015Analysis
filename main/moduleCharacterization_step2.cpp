  ...
  for(auto stepLabel : stepLabels) {
	...
	//label histo
	  ...
	  // -- draw ToT
          ...
          histo -> SetLineColor(kRed);
	  // histo -> Draw();
          TF1* f_tot = new TF1 ( Form("f_tot_%s",label.c_str()), "gaus(0)", histo->GetMean()-5.*histo->GetRMS(), histo->GetMean()-5.*histo->GetRMS());
	  f_tot -> SetNpx(10000);
	  ...
	  histo -> Draw();
          ...
          latex -> Draw("same");      
          histo -> Write();
	  std::cout << Form("f_tot_%s",label.c_str()) << " " << 0 << " " <<  f_tot->GetParameter(1) << " " << f_tot->GetParError(1) << std::endl;
  ...
  for(auto mapIt : trees)
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  if( h1_energyRatio[index2] == NULL )
	    {
	      ...
	      //h1_energyRatio[index2] = new TH1F(Form("h1_energyRatio_%s",labelLR_energyBin.c_str()),"",300,0.,5.);
              h1_energyRatio[index2] = new TH1F(Form("h1_energyRatio_%s",labelLR_energyBin.c_str()),"",1000,-5.,20.);
	      // h1_totRatio[index2] = new TH1F(Form("h1_totRatio_%s",labelLR_energyBin.c_str()),"",2000,0.,5.);
              h1_totRatio[index2] = new TH1F(Form("h1_totRatio_%s",labelLR_energyBin.c_str()),"",2000,-5.,5.);
	  ...
	  if (fabs(anEvent->timeR-anEvent->timeL)<10000)
	    {
	      if( (anEvent->energyR / anEvent->energyL > 0) & (anEvent->energyR / anEvent->energyL < 20) )
		{
	  ...
	  if( h1_deltaT_energyRatioCorr[index2] == NULL )
	    {
	      ...
	      p1_deltaT_energyRatioCorr_vs_totRatio[index2] = new TProfile(Form("p1_deltaT_energyRatioCorr_vs_totRatio_%s",labelLR_energyBin.c_str()),"",50,fitFunc_totRatio[index2]->GetParameter(1)-4.*fitFunc_totRatio[index2]->GetParameter(2), fitFunc_totRatio[index2]->GetParameter(1)+4.*fitFunc_totRatio[index2]->GetParameter(2));
	      ...
	      p1_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean[index2] = new TProfile(Form("p1_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",100,0,1000);
	    }
      ...
      for(int iBar = 0; iBar < 16; ++iBar)
	{
	  ...
	  for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
		...
		contr = 1;
	      ...
	      histo -> SetTitle(Form(";energy+tot-corrected #Deltat [ps];entries"));
	      histo -> SetLineWidth(2);
	      histo -> SetLineColor(kGreen+1);
	      histo -> SetMarkerColor(kGreen+1);
	      ...
	      latex = new TLatex(0.55,0.85,Form("#splitline{#sigma_{energy+tot.}^{eff} = %.0f ps}{#sigma_{energy+tot}^{gaus} = %.0f ps}",effSigma,fabs(fitFunc->GetParameter(2))));
              latex -> SetNDC();
	      latex -> SetTextFont(42);
	      latex -> SetTextSize(0.04);
	      latex -> SetTextColor(kGreen+2);
	      ...
	      fitFunc = new TF1(Form("fitFunc_energyRatioCorr_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
	      fitFunc -> SetParameters(1,histo->GetMean(),histo->GetRMS());
	      fitFunc -> SetRange(fitXMin, fitXMax);
	      histo -> Fit(fitFunc,"QNRSL","", fitXMin, fitXMax);
	      fitFunc -> SetRange(fitFunc->GetParameter(1)-fitFunc->GetParameter(2),fitFunc->GetParameter(1)+fitFunc->GetParameter(2));
