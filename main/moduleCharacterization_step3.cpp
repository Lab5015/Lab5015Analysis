...
double GetLinearizedEnergy(double* xx, double* par ){
	double x;
	x = xx[0];
	if(x<51) return (par[0]*x+1)*par[1]+(1-exp(-0.1*x+par[2]))*par[3];
	else 			return (par[4]*x+par[5]);
}
...
int main(int argc, char** argv)
{
  ...
  system(Form("mkdir -p %s/summaryPlots/timeResolution_ToTcorr/",plotDir.c_str()));
  ...
  for(std::map<float,float>::iterator index = map_en1275.begin(); index != map_en1275.end(); index++){
    ...
    if (index->second>0.1){
      ...
      //std::cout<<g_en1275_vs_th[Vov_iBar_ID]->GetN()<<" "<<th<<" "<<index->second<<std::endl;
...
//energy Peak 1786 vs th and vs Vov and vs iBar (LR, L, R)
  ...
  for(std::map<float,TGraphErrors*>::iterator index = g_en511_vs_th.begin(); index != g_en511_vs_th.end(); index++){
    ...
    for(int bar=0; bar<16; bar++){
      if(bar==iBar){
        if(c_en511_vs_th[bar]==NULL){
          ...
          iter[bar] = 0;
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(-1.,0.,64.,800.) );
	...
	if(c_en1275_vs_th[bar]==NULL){
          ...
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(-1.,0.,64.,300.) );
          hPad -> SetTitle(";threshold [DAC];energy [a.u.]");
	  hPad -> Draw();
	  gPad -> SetGridy();
        }
	...
	if(c_en1275_vs_th_R[bar]==NULL){
	  ...
	  gPad -> SetGridy();
        ...
        if(c_enRatio_vs_th[bar]==NULL){
          ...
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(-1.,1.,64.,10.) );
          hPad -> SetTitle(";threshold [DAC];energy [a.u.]");
          hPad -> Draw();
          gPad -> SetGridy();
        }
        ...
        if(c_en1786_vs_th[bar]==NULL && (!source.compare(SingleBarNa22) || !source.compare(Co60))){
          ...
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(-1.,150.,64.,500.) );
          hPad -> SetTitle(";threshold [DAC];energy [a.u.]");
          hPad -> Draw();
          gPad -> SetGridy();
        }
	  ...
	  gPad -> SetGridy();
        ...
        if(!source.compare("Laser")) ++iter[bar];
    ...
    for(int bar=0; bar<16; bar++){
      if(bar==iBar){
        if(c_en511_vs_Vov[bar]==NULL){
          ...
          iter[bar] = 0;
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,0.,10.,800.) );
	...
	/*if(c_en511_vs_Vov_L[bar]==NULL){
	  ...
	  hPad -> Draw();
        ...
        if(c_en1275_vs_Vov[bar]==NULL){
          ...
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,0.,10.,300.) );
          hPad -> SetTitle(";V_{ov} [V];energy [a.u.]");
          hPad -> Draw();
          gPad -> SetGridy();
        }
	...
	/*if(c_en1275_vs_Vov_L[bar]==NULL){
	  ...
	  gPad -> SetGridy();
        ...
        if(c_enRatio_vs_Vov[bar]==NULL){
          ...
          TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,1.,10.,10.) );
          hPad -> SetTitle(";V_{ov} [V];energy [a.u.]");
          hPad -> Draw();
          gPad -> SetGridy();       
        }
  ...
  /*for(std::map<float,TGraphErrors*>::iterator index = g_en511_vs_bar.begin(); index != g_en511_vs_bar.end(); index++){
    ...
    if(c_en511_vs_bar[Vov_th_ID]==NULL){
      ...
      iter[Vov_th_ID] = 0;
      TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,0.,17.,100.) );
    ...
    TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,0.,17.,300.) );
    hPad -> SetTitle(";ID bar;energy [a.u.]");
	  hPad -> Draw();
	  gPad -> SetGridy();       
	}
    ...
    TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,1.,17.,10.) );
    ...
    TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,150.,17.,200.) );
    hPad -> SetTitle(";ID bar;energy [a.u.]");
	  hPad -> Draw();
	  gPad -> SetGridy();       
	}
 ...
 */
  ...
  std::map<double,float> map_timeResToT;
  std::map<double,float> map_errTimeResToT;
  std::map<double,float> map_errTimeResToT511;
  std::map<double,float> map_errTimeResToT1275;
  ...
  std::map<double,TGraphErrors*> g_timeResToT_vs_th;
  std::map<double,TGraphErrors*> g_timeResToT_vs_Vov;
  std::map<double,TGraphErrors*> g_timeRes511ToT_vs_bar;
  std::map<double,TGraphErrors*> g_timeRes1275ToT_vs_bar;
  std::map<double,TGraphErrors*> g_timeResToT_vs_enBin;
...
//NB:attenzione qui ancora da cambiare per Na22 Co60, Co60SumPeak e Laser
  ...
  if(!source.compare(Co60SumPeak) || !source.compare(Laser) || !source.compare(TB)){
    enBin511=1;//SumPeak
    enBin1275=-1;//empty
  ...
  for (auto mapIt : trees2){
    ...
    mapIt.second -> SetBranchStatus("timeResolutionToT",  1); mapIt.second -> SetBranchAddress("timeResolutionToT",  &timeResToT);
    ...
    mapIt.second -> SetBranchStatus("errTimeResolutionToT",  1); mapIt.second -> SetBranchAddress("errTimeResolutionToT",  &errTimeResToT);
    ...
    if(timeRes > -1 && timeResToT > -1){  
      map_timeRes[theIndex2] = timeRes;
      map_timeResToT[theIndex2] = timeResToT;
      map_enBin[theIndex2] = enBin;
      map_errTimeRes[theIndex2] = errTimeRes/2;
      map_errTimeResToT[theIndex2] = errTimeResToT/2;
      if (int(theIndex2/10000000) == enBin511){
        ...
        map_errTimeResToT511[theIndex2] = errTimeResToT/2;
      }
      if (int(theIndex2/10000000) == enBin1275){
        ...
        map_errTimeResToT1275[theIndex2] = errTimeResToT/2;
      }      
    }
  ...
  for(std::map<double,float>::iterator index = map_timeRes.begin(); index != map_timeRes.end(); index++){
    ...
    Vov = (int(double(index->first-enBin*10000000)/10000.)/100.);
    std::cout<<"qui "<<Vov*1000000<<std::endl;
    th = int((index->first - enBin*10000000 - int(Vov*100)*10000)/100.);
    //if (Vov <1.3 && Vov>1.1) th = th +1;
    iBar = int(double(index->first - enBin*10000000 - int(Vov*100)*10000 - th*100));
    ...
    std::cout<<"index "<<index->first<<"   "<<"th= "<<th<<"   "<<"Vov= "<<Vov<<"   "<<std::endl;
    ...
    if( g_timeResToT_vs_th[Vov_iBar_enBin_ID] == NULL ){
      g_timeResToT_vs_th[Vov_iBar_enBin_ID] = new TGraphErrors();
    }
    ...
    if( g_timeResToT_vs_Vov[th_iBar_enBin_ID] == NULL ){	
      g_timeResToT_vs_Vov[th_iBar_enBin_ID] = new TGraphErrors();
    }
    if( g_timeResToT_vs_enBin[Vov_th_iBar_ID] == NULL ){	
      g_timeResToT_vs_enBin[Vov_th_iBar_ID] = new TGraphErrors();
    }
    ...
    if (enBin ==enBin511){
      ...
      if( g_timeRes511ToT_vs_bar[Vov_th_ID] == NULL ){	
      	g_timeRes511ToT_vs_bar[Vov_th_ID] = new TGraphErrors();
      }
    ...
    if (enBin ==enBin1275){
      ...
      if( g_timeRes1275ToT_vs_bar[Vov_th_ID] == NULL ){	
      	g_timeRes1275ToT_vs_bar[Vov_th_ID] = new TGraphErrors();
      }
    }
    ...
    if (index->second>-1){
      ...
      g_timeResToT_vs_th[Vov_iBar_enBin_ID]->SetPoint(g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetN(), th, (map_timeResToT[index->first]/2.));
      //std::cout<<"N"<<g_timeRes_vs_th[Vov_iBar_enBin_ID]->GetN()<<"th"<<th<<"RES "<<(index->second)/2<<"   VovIbarenBinID"<<Vov_iBar_enBin_ID<<"Vov"<<Vov<<std::endl;
      g_timeResToT_vs_th[Vov_iBar_enBin_ID]->SetPointError(g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetN()-1, 0., 0.);
      ...
      g_timeResToT_vs_Vov[th_iBar_enBin_ID]->SetPoint(g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetN(), Vov, (map_timeResToT[index->first]/2.));
      //std::cout<<"N"<<g_timeRes_vs_Vov[th_iBar_enBin_ID]->GetN()<<"th"<<th<<"RES "<<(index->second)/2<<"   thIbarenBinID"<<th_iBar_enBin_ID<<"Vov"<<Vov<<std::endl;
      g_timeResToT_vs_Vov[th_iBar_enBin_ID]->SetPointError(g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetN()-1, 0., 0.);
      ...
      tResToT_err = map_errTimeResToT[index->first];
      
      g_timeResToT_vs_enBin[Vov_th_iBar_ID]->SetPoint(g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetN(), map_enBin[index->first], (map_timeResToT[index->first]/2.));
      
      
      //!!!!IN CASO CAMBIA QUI
      g_timeResToT_vs_enBin[Vov_th_iBar_ID]->SetPointError(g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetN()-1, map_enBin[index->first]*energy_err_sys, tResToT_err);
      //std::cout<<"errX"<<map_enBin[index->first]*energy_err_sys<<"    errY "<<tRes_err<<std::endl;
    }
    ...
    if (enBin ==enBin511){
      ...
      g_timeRes511ToT_vs_bar[Vov_th_ID]->SetPoint(g_timeRes511ToT_vs_bar[Vov_th_ID]->GetN(), iBar, (map_timeResToT[index->first]/2.));
      //std::cout<<"N"<<g_timeRes511_vs_bar[Vov_th_ID]->GetN()<<"iBar"<<iBar<<"RES "<<(index->second)/2<<std::endl;
      g_timeRes511ToT_vs_bar[Vov_th_ID]->SetPointError(g_timeRes511ToT_vs_bar[Vov_th_ID]->GetN()-1, 0., 0.);
    }
    ...
    if (enBin ==enBin1275){
      ...
      g_timeRes1275ToT_vs_bar[Vov_th_ID]->SetPoint(g_timeRes1275ToT_vs_bar[Vov_th_ID]->GetN(), iBar, (map_timeResToT[index->first]/2.));
      //std::cout<<"N"<<g_timeRes1275_vs_bar[Vov_th_ID]->GetN()<<"iBar"<<iBar<<"RES "<<(index->second)/2<<std::endl;
      g_timeRes1275ToT_vs_bar[Vov_th_ID]->SetPointError(g_timeRes1275ToT_vs_bar[Vov_th_ID]->GetN()-1, 0., 0.);
    }
  }
  ...
  std::map<double, TCanvas*> c_timeResToT_vs_th;
  ...
  std::map<double, TCanvas*> c_timeResToT_vs_Vov;
  std::map<double, TCanvas*> c_timeResToT_vs_enBin;
  std::map<double, TCanvas*> c_timeResToT_vs_bar;
  std::map<double, TCanvas*> c_timeRes_vs_enBin_thBest;
  std::map<double, TCanvas*> c_timeResToT_vs_enBin_thBest;
        ...
        for(int energyBin=1; energyBin<enBin1275+t; energyBin++){
          if(energyBin==enBin){
            ...
            if(c_timeRes_vs_th[iBar_enBin_ID]==NULL){
              ...
              TH1F* hPad = (TH1F*)( gPad->DrawFrame(-1.,0,64.,200) );
            ...
            outFile -> cd();
            g_tRes -> Write(Form("g_timeRes_vs_th_Vov%.1f_bar%02d_enBin%d",Vov,bar,energyBin));
            std::cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> writing " << Form("g_timeRes_vs_th_Vov%.1f_bar%02d_enBin%d",Vov,bar,energyBin) << std::endl;
...
//summary plot time resolution ToT corrected vs th
  for(std::map<double,TGraphErrors*>::iterator index = g_timeResToT_vs_th.begin(); index != g_timeResToT_vs_th.end(); index++){
	...
	//ATTENZIONE!!!!!!!!
        for(int energyBin=1; energyBin<enBin1275+t; energyBin++){
          if(energyBin==enBin){
            //if(enBin==enBin511 || enBin==enBin1275){  
            if(c_timeResToT_vs_th[iBar_enBin_ID]==NULL){
              c_timeResToT_vs_th[iBar_enBin_ID] = new TCanvas(Form("c_timeResToT_vs_th_bar%02d_enBin%02d",bar,energyBin),Form("c_timeResToT_vs_th_bar%02d_enBin%02d",bar,energyBin));
            ...
            //}
            TGraph* g_tRes = g_timeResToT_vs_th[Vov_iBar_enBin_ID];
            //outFile -> cd();
            //g_tRes -> Write(Form("g_timeRes_vs_th_Vov%.1f_bar%02d_enBin%d",Vov,bar,energyBin));
            //if(enBin==enBin511 || enBin==enBin1275){
            c_timeResToT_vs_th[iBar_enBin_ID]->cd();
            //TGraph* g_tRes = g_timeRes_vs_th[Vov_iBar_enBin_ID];
            std::cout<< g_tRes -> GetN()<<std::endl; 
            g_tRes -> SetLineColor(1+iter[iBar_enBin_ID]);
            g_tRes -> SetMarkerColor(1+iter[iBar_enBin_ID]);
            ...
            g_tRes -> Draw("PL,same");
            outFile -> cd();
            g_tRes -> Write(Form("g_timeResToT_vs_th_Vov%.1f_bar%02d_enBin%d",Vov,bar,energyBin));
            ...
            for (float over : vec_Vov){
              if (over == Vov){
                double val = 9999;
                int xVal = 0;
                for ( int nPoint = 0; nPoint < g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetN(); nPoint++){
                  if ( g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetPointY(nPoint)< val && g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetPointY(nPoint)>0 ){
                    val = g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetPointY(nPoint);
                    xVal = nPoint;
                  }
                  tBest[Vov_iBar_enBin_ID] = g_timeResToT_vs_th[Vov_iBar_enBin_ID]->GetPointX(xVal);
                }																				
              }	  		  
            }
          }		
  ...
  for(std::map<double,TCanvas*>::iterator index = c_timeResToT_vs_th.begin(); index != c_timeResToT_vs_th.end(); index++){
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_th_bar%02d_enBin%02d.png",plotDir.c_str(),int(index->first-double((int(index->first/10000000))*10000000)), int(double(index->first/10000000.))));
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_th_bar%02d_enBin%02d.pdf",plotDir.c_str(),int(index->first-double((int(index->first/10000000))*10000000)), int(double(index->first/10000000.))));
  }
          ...
          if(energyBin==enBin){
            if(enBin==enBin511 || enBin==enBin1275){
              for ( int nPoint = 0; nPoint < g_timeRes_vs_Vov[th_iBar_enBin_ID]->GetN(); nPoint++){
                ...
                //std::cout<<"y "<<g_timeRes_vs_Vov[th_iBar_enBin_ID]->GetPointY(nPoint)<<std::endl;
                if(th == tBest[double(10000000*enBin) + double(over*1000000) + double(iBar)]){
                  ...
                  //g_tRes -> SetLineColor(1+iter[iBar_enBin_ID]);
                  //g_tRes -> SetMarkerColor(kBlack+iter[iBar_enBin_ID]);
                  g_tRes -> SetMarkerStyle(20);
                  g_tRes -> Draw("P,same");
                  outFile -> cd();
  ...
  //summary plot time resolution ToT corrected vs Vov
  for(std::map<double,TGraphErrors*>::iterator index = g_timeResToT_vs_Vov.begin(); index != g_timeResToT_vs_Vov.end(); index++){
	...
	//ATTENZIONE!!!!
        for(int energyBin=1; energyBin<enBin1275+t; energyBin++){
          if(energyBin==enBin){
            if(enBin==enBin511 || enBin==enBin1275){
              for ( int nPoint = 0; nPoint < g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetN(); nPoint++){
                float over = g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetPointX(nPoint);
                //std::cout<<"over "<<over<<std::endl;
                //std::cout<<"y "<<g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetPointY(nPoint)<<std::endl;
                if(th == tBest[double(10000000*enBin) + double(over*1000000) + double(iBar)]){
                  //if(th == refTh){
                  if( g_new[iBar_enBin_ID] == NULL ) g_new[iBar_enBin_ID] = new TGraphErrors();
                  g_new[iBar_enBin_ID]->SetPoint(g_new[iBar_enBin_ID]->GetN(), g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetPointX(nPoint), g_timeResToT_vs_Vov[th_iBar_enBin_ID]->GetPointY(nPoint));
                  if(c_timeResToT_vs_Vov[iBar_enBin_ID]==NULL){
                    c_timeResToT_vs_Vov[iBar_enBin_ID] = new TCanvas(Form("c_timeResToT_vs_Vov_bar%02d_enBin%02d",bar,energyBin),Form("c_timeResToT_vs_Vov_bar%02d_enBin%02d",bar,energyBin));
                    iter[iBar_enBin_ID] = 0;
                    TH1F* hPad = (TH1F*)( gPad->DrawFrame(0,0,10.,150) );
                    hPad -> SetTitle(";V_{ov} [V];#sigma_{t_{Diff}}/2 [ps]");
                    hPad -> Draw();
                  ...
                  c_timeResToT_vs_Vov[iBar_enBin_ID]->cd();
                  TGraph* g_tRes = g_new[iBar_enBin_ID];
                  ...
                  g_tRes -> Draw("P,same");
                  outFile -> cd();
                  g_tRes -> Write(Form("g_timeResToT_vs_Vov_th%d_bar%02d_enBin%d",th,bar,energyBin));
  ...
  for(std::map<double,TCanvas*>::iterator index = c_timeResToT_vs_Vov.begin(); index != c_timeResToT_vs_Vov.end(); index++){
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_Vov_bar%02d_enBin%02d.png",plotDir.c_str(),int(index->first-double((int(index->first/10000000))*10000000)), int(double(index->first/10000000.))));
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_Vov_bar%02d_enBin%02d.pdf",plotDir.c_str(),int(index->first-double((int(index->first/10000000))*10000000)), int(double(index->first/10000000.))));
  }
            ...
            for(int l=0; l<vec_th.size(); l++){
              if(vec_th[l]==th){
                if(c_timeRes_vs_enBin[Vov_th_iBar_ID]==NULL){
                  ...
                  TH1F* hPad = (TH1F*)( gPad->DrawFrame(0,0,800., 1.1*g_timeRes_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()) );
                  //std::cout<<"maximum "<<1.1*g_timeRes_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()<<std::endl;
                ...
                if(c_timeRes_vs_enBin_thBest[iBar_Vov_ID]==NULL){
                  ...
                  TH1F* hPad = (TH1F*)( gPad->DrawFrame(0,0,800., 1.1*g_timeRes_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()) );
                  //std::cout<<""<<1.1*g_timeRes_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()<<std::endl;
                  hPad -> SetTitle(";energy [a.u.];#sigma_{t_{Diff}}/2 [ps]");
                  hPad -> Draw();
                  gPad -> SetGridy();
                }
                ...
                //ATTENZIONE!!!: QUI CAMBIATO PER RUN 126 SPECIFICO, NON HO TH20, DA AGGIORNARE A SECONDA DELLE TH CHE AVREMO E QUELLA CHE SARÀ DI REF
                if(th == refTh){
  ...
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT;
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT_error;
  std::map<float, std::map<int, TGraph*>> g_tResToT_vs_energy_thRef;
  std::map<float, std::map<int, TGraph*>> g_tResToT_vs_energy_th20;
  
  
//summary plot time resolution ToT corrected vs energy bin
  for(std::map<double,TGraphErrors*>::iterator index = g_timeResToT_vs_enBin.begin(); index != g_timeResToT_vs_enBin.end(); index++){
      ...
      if(bar==iBar){
        for(int i=0; i<vec_Vov.size(); i++){
          if(vec_Vov[i]==Vov){
            for(int l=0; l<vec_th.size(); l++){
              if(vec_th[l]==th){
                if(c_timeResToT_vs_enBin[Vov_th_iBar_ID]==NULL){
                  c_timeResToT_vs_enBin[Vov_th_iBar_ID] = new TCanvas(Form("c_timeResToT_vs_enBin_bar%02d_Vov%.01f_th%d",bar,Vov,th),Form("c_timeResToT_vs_enBin_bar%02d_Vov%.01f_th%d",bar,Vov,th));
                  TH1F* hPad = (TH1F*)( gPad->DrawFrame(0,0,800., 1.1*g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()) );
                  //std::cout<<"maximum "<<1.1*g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()<<std::endl;
                  hPad -> SetTitle(";energy [a.u.];#sigma_{t_{Diff}}/2 [ps]");
                  hPad -> Draw();
                  gPad -> SetGridy();
                }
                if(c_timeResToT_vs_enBin_thBest[iBar_Vov_ID]==NULL){
                  c_timeResToT_vs_enBin_thBest[iBar_Vov_ID] = new TCanvas(Form("c_timeResToT_vs_enBin_bar%02d_Vov%.01f_bestTh",bar,Vov),Form("c_timeResToT_vs_enBin_bar%02d_Vov%.01f_bestTh",bar,Vov));
                  TH1F* hPad = (TH1F*)( gPad->DrawFrame(0,0,800., 1.1*g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()) );
                  //std::cout<<""<<1.1*g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetHistogram()->GetMaximum()<<std::endl;
                  hPad -> SetTitle(";energy [a.u.];#sigma_{t_{Diff}}/2 [ps]");
                  hPad -> Draw();
                  gPad -> SetGridy();
                }
                ...
                c_timeResToT_vs_enBin[Vov_th_iBar_ID]->cd();
                TGraph* g_tRes_en = g_timeResToT_vs_enBin[Vov_th_iBar_ID];
                ...
                g_tRes_en -> Draw("PL");
                outFile -> cd();
                g_tRes_en -> Write(Form("g_timeResToT_vs_enBin_bar%02d_Vov%.01f_th%d",iBar,Vov, th));
                
                //ATTENZIONE!!!: QUI CAMBIATO PER RUN 126 SPECIFICO, NON HO TH20, DA AGGIORNARE A SECONDA DELLE TH CHE AVREMO E QUELLA CHE SARÀ DI REF
                if(th == refTh){
                  g_tRes_vs_energy_th20[Vov][iBar] = g_tRes_en;
                  g_tRes_vs_energy_thRef[Vov][iBar] = g_tRes_en;
                  g_tRes_en -> Write(Form("g_timeResToT_vs_enBin_bar%02d_Vov%.01f_thRef%02d",iBar,Vov, th));
                }
                ...
                for ( int nPoint = 0; nPoint < g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetN(); nPoint++){
                  float eBin = g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetPointX(nPoint);
                  if(vec_th[l] == tBest[double(10000000*(nPoint+1)) + double(Vov*1000000) + double(bar)] ){
                    vec_tResToT[iBar_Vov_ID].push_back(std::make_pair ( eBin, g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetPointY(nPoint)));
                    vec_tResToT_error[iBar_Vov_ID].push_back(std::make_pair ( g_timeResToT_vs_enBin[Vov_th_iBar_ID]->GetErrorX(nPoint), g_timeResToT_vs_enBin[Vov_th_iBar_ID] ->GetErrorY(nPoint))); 				            
                  }
                }		
              }
            }
  ...
  for(std::map<double,TCanvas*>::iterator index = c_timeResToT_vs_enBin.begin(); index != c_timeResToT_vs_enBin.end(); index++){
    Vov = float(int(index->first/10000.)/100.);
    th = int(double(index->first - Vov*1000000)/100.);
    iBar = int(double(index->first - Vov*1000000 - th*100));
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_enBin_bar%02d_Vov%.01f_th%d.png",plotDir.c_str(),iBar,Vov, th));
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_enBin_bar%02d_Vov%.01f_th%d.pdf",plotDir.c_str(),iBar,Vov, th));
  }
  std::map<float, std::map<int, TGraph*>> g_tResToT_vs_energy;
  for(std::map<double,TCanvas*>::iterator index = c_timeResToT_vs_enBin_thBest.begin(); index != c_timeResToT_vs_enBin_thBest.end(); index++){
    index->second -> cd();
     Vov = float(int(index->first/10000.)/100.);
     iBar = int(double(index->first - Vov*1000000));
     TGraphErrors * graph = new TGraphErrors();
     int up = vec_tResToT[index->first].size();
     for (int i = 0; i < up; i++){
       float min = 9999;
       int minIndex = 0;
       for (int j = 0; j < vec_tResToT[index->first].size(); j++){
         if ( vec_tResToT[index->first][j].first < min && vec_tResToT[index->first][j].first>0){
           min = vec_tResToT[index->first][j].first;
           minIndex = j;
         }
       }
       graph -> SetPoint( i, vec_tResToT[index->first][minIndex].first, vec_tResToT[index->first][minIndex].second);
       graph -> SetPointError( i, vec_tResToT_error[index->first][minIndex].first, vec_tResToT_error[index->first][minIndex].second);
       vec_tResToT[index->first].erase(vec_tResToT[index->first].begin() + minIndex);
       vec_tResToT_error[index->first].erase(vec_tResToT_error[index->first].begin() + minIndex);
     }
     graph-> SetLineColor(kBlack);
     graph-> SetMarkerColor(kBlack);
    graph -> SetMarkerStyle(20);
    graph-> Draw("PL");
    outFile -> cd();
    graph -> Write(Form("g_timeResToT_vs_enBin_bar%02d_Vov%.01f_bestTh",iBar,Vov));
    g_tResToT_vs_energy[Vov][iBar] = graph;
    
    
    
    index->second-> Print(Form("%s/summaryPlots/timeResolution/c_timeResToT_vs_enBin_bar%02d_Vov%.01f_bestTh.png",plotDir.c_str(),iBar,Vov));
    index->second-> Print(Form("%s/summaryPlots/timeResolution/c_timeResToT_vs_enBin_bar%02d_Vov%.01f_bestTh.pdf",plotDir.c_str(),iBar,Vov));
  }
  ...
  for(std::map<double,TCanvas*>::iterator index = c_timeRes_vs_bar.begin(); index != c_timeRes_vs_bar.end(); index++){
    ...
    if(!source.compare("TB")) Label1 = "MIP Peak";
...
//summary plot time resolution ToT coorected 511 peak and 1275 peak vs iBar
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT_511;
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT_error_511;
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT_error_1275;
  std::map<double, std::vector<std::pair<float,float>>>vec_tResToT_1275;
  for(std::map<double,TGraphErrors*>::iterator index = g_timeRes511ToT_vs_bar.begin(); index != g_timeRes511ToT_vs_bar.end(); index++){
    ...
    if(c_timeResToT_vs_bar[Vov]==NULL){
      c_timeResToT_vs_bar[Vov] = new TCanvas(Form("c_timeResToT_vs_bar_Vov%.01f",Vov),Form("c_timeResToT_vs_bar_Vov%.01f",Vov));
      c_timeResToT_vs_bar[Vov]->cd();
      TH1F* hPad = (TH1F*)( gPad->DrawFrame(0.,0.,17.,200.) );
      hPad -> SetTitle(";ID bar;#sigma_{t_{Diff}}/2 [ps]");
      hPad -> Draw();
      gPad -> SetGridy();    
    }
    ...
    TGraph* g_timeRes511ToT = g_timeRes511ToT_vs_bar[Vov_th_ID];
    
    
    
    for ( int nPoint = 0; nPoint < g_timeRes511ToT->GetN(); nPoint++){ 
      if (th == tBest[ double(10000000*enBin511) + double(Vov*1000000) + double(int(g_timeRes511ToT->GetPointX(nPoint)))]){
        vec_tResToT_511[Vov].push_back(std::make_pair ( int(g_timeRes511ToT->GetPointX(nPoint)), g_timeRes511ToT->GetPointY(nPoint)));
        vec_tResToT_error_511[Vov].push_back(std::make_pair ( g_timeRes511ToT->GetErrorX(nPoint), g_timeRes511ToT->GetErrorY(nPoint)));             
      }
    }
    ...
    if (g_timeRes1275ToT_vs_bar[Vov_th_ID]!=NULL){
      TGraph* g_timeRes1275ToT = g_timeRes1275ToT_vs_bar[Vov_th_ID];  
      for ( int nPoint = 0; nPoint < g_timeRes1275ToT->GetN(); nPoint++){
        if (th == tBest[ double(10000000*enBin1275) + double(Vov*1000000) + double(int(g_timeRes1275ToT->GetPointX(nPoint)))]){
          vec_tResToT_1275[Vov].push_back(std::make_pair ( int(g_timeRes1275ToT->GetPointX(nPoint)), g_timeRes1275ToT->GetPointY(nPoint)));
          vec_tResToT_error_1275[Vov].push_back(std::make_pair ( g_timeRes1275ToT->GetErrorX(nPoint), g_timeRes1275ToT->GetErrorY(nPoint)));   
        }
      }  
    }
  } 
  ...
  for(std::map<double,TCanvas*>::iterator index = c_timeResToT_vs_bar.begin(); index != c_timeResToT_vs_bar.end(); index++){
   
    index->second -> cd();
    //Vov = index->first;
    TGraphErrors * graph1 = new TGraphErrors();
    TGraphErrors * graph2 = new TGraphErrors();
    int up = vec_tResToT_511[index->first].size();   
    for (int i = 0; i < up; i++){
      float min = 9999;
      int minIndex = 0;
      for (int j = 0; j < vec_tResToT_511[index->first].size(); j++){
        if (vec_tResToT_511[index->first][j].first < min){
          min = vec_tResToT_511[index->first][j].first;
          minIndex = j;
        }
      }
      graph1 -> SetPoint( i, vec_tResToT_511[index->first][minIndex].first, vec_tResToT_511[index->first][minIndex].second);
      graph1 -> SetPointError( i, vec_tResToT_error_511[index->first][minIndex].first, vec_tResToT_error_511[index->first][minIndex].second);
      vec_tResToT_511[index->first].erase(vec_tResToT_511[index->first].begin() + minIndex);
      vec_tResToT_error_511[index->first].erase(vec_tResToT_error_511[index->first].begin() + minIndex);
    }
    ...
    graph1-> Draw("PL,same");
    outFile -> cd();
    graph1 -> Write(Form("g_timeRes511ToT_vs_bar_Vov%.01f_bestTh",Vov));
    ...
    if(g_timeRes1275ToT_vs_bar[Vov_th_ID]!=NULL){
      int up2 = vec_tResToT_1275[index->first].size();
      for (int k = 0; k < up2; k++){
        float min = 9999;
        int minIndex = 0;
        for (int l = 0; l < vec_tResToT_1275[index->first].size(); l++){
          if (vec_tResToT_1275[index->first][l].first < min){
            min = vec_tResToT_1275[index->first][l].first;
            minIndex = l;
          }
        }
        graph2 -> SetPoint( k, vec_tResToT_1275[index->first][minIndex].first, vec_tResToT_1275[index->first][minIndex].second);
        graph2 -> SetPointError( k, vec_tResToT_error_1275[index->first][minIndex].first, vec_tResToT_error_1275[index->first][minIndex].second);
        vec_tResToT_1275[index->first].erase(vec_tResToT_1275[index->first].begin() + minIndex);
        vec_tResToT_error_1275[index->first].erase(vec_tResToT_error_1275[index->first].begin() + minIndex);
      }
      graph2-> SetLineColor(kBlue);
      graph2-> SetMarkerColor(kBlue);
      graph2 -> SetMarkerStyle(20);
      graph2-> Draw("PL, same");
      outFile -> cd();
      graph2 -> Write(Form("g_timeRes1275ToT_vs_bar_Vov%.01f_bestTh",Vov));
    ...
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_bar_Vov%.01f.png",plotDir.c_str(),float(index->first)));
    index->second-> Print(Form("%s/summaryPlots/timeResolution_ToTcorr/c_timeResToT_vs_bar_Vov%.01f.pdf",plotDir.c_str(),float(index->first)));
  }
    ...
    for( float Vov : vec_Vov){
      ...
      for(int iBar = 0; iBar < 16; ++iBar){
        ...
        for(int point = 0; point < g_energy511_vs_Vov[refTh][iBar]->GetN(); ++point){
          ...
          if (ov==1 || ov==2 || ov==3 || ov==4) continue;
        ...
        TH1F* hPad1 = (TH1F*)( gPad->DrawFrame(0.,0.,300.,7.) );
        ...
        //TF1* f_linearity = new TF1(Form("f_linearity_Vov%.01f", Vov),"[0]*(exp([1]*x)-1) + [2]",0.,500.);
        TF1* f_linearity = new TF1(Form("f_linearity_Vov%.01f", Vov),"pol3",0.,500.);
        ...
        for(int point = 0; point < g_tRes_vs_energy[Vov][iBar]->GetN(); ++point){
          ...
          //x = g_tRes_vs_energy[Vov][iBar] -> GetPointX(point);
        ...
        for(int point = 0; point < g_tRes_vs_energy_thRef[Vov][iBar]->GetN(); ++point){
          double x,y;
          x = g_tRes_vs_energy_th20[Vov][iBar] -> GetPointX(point);
          //x = g_tRes_vs_energy_th20[Vov][iBar] -> GetPointX(point);
...
//energy linearization normalization 3.5
  ...
  */
  ...
  if(!source.compare(Na22) || !source.compare(Co60) || !source.compare(SingleBarNa22)){
    for( float Vov : vec_Vov){
      if (Vov==1 || Vov==2 || Vov==3 || Vov==4) continue;
      ...
      for(int iBar = 0; iBar < 16; ++iBar){
        ...
        g_linearity -> SetPoint(g_linearity->GetN(),0,0);
        ...
        for(int point = 0; point < g_energy511_vs_Vov[refTh][iBar]->GetN(); ++point){
          ...
          if(!source.compare(Na22) || !source.compare(SingleBarNa22)){
            ...
            energy_scaled = 0.511 * ( PDE_vs_OV(ov,SiPMType) * Gain_vs_OV(ov,SiPMType) * ECF_vs_OV(ov,SiPMType) ) /  ( PDE_vs_OV(3.5,SiPMType) * Gain_vs_OV(3.5,SiPMType) * ECF_vs_OV(3.5,SiPMType) ) / corr_511 ;
            
            std::cout<<"energy scaled "<<energy_scaled<<std::endl;
            //std::cout<<"Npe "<<Npe_511<<std::endl;
            //std::cout<<"PDE_vs_OV(ov, SiPMType) "<<double(PDE_vs_OV(ov, SiPMType))<<std::endl;
        ...
        for(int point = 0; point < g_energy1275_vs_Vov[refTh][iBar]->GetN(); ++point){
          ...
          if(!source.compare(Na22) || !source.compare(SingleBarNa22)){
            ...
            energy_scaled = 1.275 * ( PDE_vs_OV(ov,SiPMType) * Gain_vs_OV(ov,SiPMType) * ECF_vs_OV(ov,SiPMType) ) / ( PDE_vs_OV(3.5,SiPMType) * Gain_vs_OV(3.5,SiPMType) * ECF_vs_OV(3.5,SiPMType) ) /corr_1275 ;
        ...
        if(!source.compare(SingleBarNa22)){
          for(int point = 0; point < g_energy1786_vs_Vov[refTh][iBar]->GetN(); ++point){
            ...
            energy_scaled = 1.786 * ( PDE_vs_OV(ov,SiPMType) * Gain_vs_OV(ov,SiPMType) * ECF_vs_OV(ov,SiPMType) ) / ( PDE_vs_OV(3.5,SiPMType) * Gain_vs_OV(3.5,SiPMType) * ECF_vs_OV(3.5,SiPMType) ) /corr_1786 ;
        ...
        TCanvas* c1 = new TCanvas(Form("c_bar%02d_%s",iBar, label.c_str()),Form("c_bar%02d_%s",iBar, label.c_str()),1300,600);
        c1->cd();
        
        TH1F* hPad1 = (TH1F*)( gPad->DrawFrame(0.,0.,300.,7.) );
        hPad1 -> SetTitle(Form(";TOFHIR2 energy (ADC);E_{MeV} #times G #times PDE #times ECF #times k_{sat}/[G #times PDE #times ECF]_{%.1f V}",3.5));
        hPad1 -> GetYaxis() -> SetTitleSize(0.04);
        hPad1 -> Draw();
        gPad -> SetGridx();
        gPad -> SetGridy();
        ...
        TF1* f_linearity = new TF1(Form("f_linearity_Vov%.01f", 3.5),GetLinearizedEnergy,0,500., 6);
	
        
        g_linearity -> Fit(f_linearity,"RS+");
        f_linearity -> Draw("same");
        outFile->cd();
        g_linearity->Write(Form("g_linearity_Vov%.01f", 3.5));
        f_linearity -> Write();
        ...
        c1->Update();
        outFile->cd();
        c1->Write(Form("c_tRes_vs_energy_%s_bar%.2d_Vov%.1f.png",label.c_str(),iBar, Vov));
        c1-> Print(Form("%s/summaryPlots/Energy_linearization/c_tRes_vs_energy_bar%.2d_Vov%.1f_%s.png",plotDir.c_str(),iBar, 3.5,label.c_str()));
        c1-> Print(Form("%s/summaryPlots/Energy_linearization/c_tRes_vs_energy_bar%.2d_Vov%.1f_%s.pdf",plotDir.c_str(),iBar, 3.5, label.c_str()));
