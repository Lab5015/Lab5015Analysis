...
int main(int argc, char** argv)
{
  ...
  system(Form("mkdir -p %s/t1fine/",plotDir.c_str()));
    system(Form("mkdir -p %s/time/",plotDir.c_str()));
  ...
  for(auto stepLabel : stepLabels) {
	...
	//printf("vth=%d\t bar= %d\n", int(vth1), iBar);
	  ...
	  //}
          ...
          // -- draw time
          c = new TCanvas(Form("c_time_%s",label.c_str()),Form("c_time_%s",label.c_str()));
          gPad -> SetLogy();
          
          histo = (TH1F*)( inFile->Get(Form("h1_time_%s",label.c_str())) );
          histo -> SetTitle(";Time [ns];entries");
          histo -> SetLineColor(kRed);
          histo -> Draw();
          // TLine* line_totAcc1 = new TLine(cut_totAcc[chID][Vov],histo->GetMinimum(),cut_totAcc[chID][Vov],histo->GetMaximum());
          // line_totAcc1 -> SetLineColor(kBlack);
          ...
          latex -> Draw("same");
          histo -> Write();
          c -> Print(Form("%s/time/c_time__%s.png",plotDir.c_str(),label.c_str()));
          c -> Print(Form("%s/time/c_time__%s.pdf",plotDir.c_str(),label.c_str()));
          delete c;
	  }
