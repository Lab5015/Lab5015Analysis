...
template<> inline std::vector<unsigned int> CfgManager::GetOpt(std::string key, int opt)
{
    key = "opts."+key;
    Errors(key, opt);
...
template<> inline std::vector<int> CfgManager::GetOpt(std::string key, int opt)
{
    key = "opts."+key;
    Errors(key, opt);
