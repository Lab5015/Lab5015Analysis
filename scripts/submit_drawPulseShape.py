...
for intrun in runs:
   ...
   with open(args.baseFolder+'/'+args.configFile) as fi:
      ...
      os.system(command)
      command = 'sed -i \"s%^step1FileName .*$%step1FileName '+args.baseFolder+'/plots_fede/pulseShape_run'+str(run)+'.root%\" '+configFileName
      os.system(command)
      command = 'sed -i \"s%^outFileNameStep1 .*$%outFileNameStep1 '+args.baseFolder+'/plots_fede/pulseShape_run'+str(run)+'.root%\" '+configFileName
      os.system(command)
      command = 'sed -i \"s%^outFileNameStep2 .*$%outFileNameStep2 '+args.baseFolder+'/plots_fede/pulseShape_run'+str(run)+'.root%\" '+configFileName
      os.system(command)
