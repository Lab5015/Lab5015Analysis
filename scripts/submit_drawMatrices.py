...
parser.add_argument("-t",  "--ith",          required=True, type=str, help="threshold type (vth1 or vth2)")
...
parallelCommand = "parallel --bar --jobs "+str(args.numParal)+" --results ./jobs/"+args.label+"/ "+args.baseFolder+"/"+args.exeName+" ::: "
...
for run in runs:
   ...
   with open(args.baseFolder + '/'+args.configFile) as fi:
      ...
      os.system(command)
      command = 'sed -i \"s%^ithMode .*$%ithMode '+args.ith+'%\" '+configFileName
      os.system(command)
      command = 'sed -i \"s%^ithMode .*$%ithMode '+args.ith+'%\" '+configFileName
      os.system(command)
      command = 'sed -i \"s%^outputLabel .*$%outputLabel '+'run'+str(run)+ '%\" '+configFileName
