...
for sipm in sipmTypes:
    ...
    for bar in bars[sipm]:
        ...
        for ov in Vovs[sipm]:
            ...
            gNoise[sipm][bar].SetPoint(gNoise[sipm][bar].GetN(), ovEff, sigma_noise(sr))
...
for sipm in sipmTypes:
    ...
    for i,bar in enumerate(bars[sipm]):
        ...
        c1[sipm][bar].cd()
        hdummy1[sipm][bar] = ROOT.TH2F('hdummy1_%s_%d'%(sipm,bar),'',100,1,2.5,180,0,180)
...
for i,bar in enumerate(bars[sipm]):
    ...
    c3[bar].SetGridy()
    c3[bar].cd()
    hdummy3[bar] = ROOT.TH2F('hdummy3_%d'%(bar),'',100,1,2.5,100,0,15)
    ...
    c4[bar].SetGridy()
    c4[bar].cd()
    hdummy4[bar] = ROOT.TH2F('hdummy4_%d'%(bar),'',100,1,2.5,100,0,20)
...
for ov in Vovs[sipm]:
    ...
    c5[ov].SetGridy()
    c5[ov].cd()
    hdummy5[ov] = ROOT.TH2F('hdummy5_%d'%(bar),'',100,-0.5,15.5,100,0,15)
