...
def getTimeResolution(h1_deltaT,hit):
   ...
   if(False):
    ...
    legend.AddEntry(fitFunc, "single gauss", "l")
    legend.Draw()
    c.Update()
...
# INPUT
inputdir ='/home/cmsdaq/ES_TB/Lab5015Analysis/TB_CERN_Sept2025/Lab5015Analysis/plots/'
source = 'TB'
...
##### NB very hardcoded, I choose the good bars with good energy spectra for TB Sept25
...
if(new_selection):
        ...
        for thr in thresholds:
            for enBin in enBins:
                ...
                for hit in Hits:
                    ...
                    g_deltaT_thBest_vs_Vovs[enBin,hit]=ROOT.TGraphErrors()
                    ...
                    for enBin in enBins:
                        for hit in Hits:
                            ...
                            if not h1_deltaT_Corr:
                                ...
                                #print("Warning: histogram not found for bar " + str(bar) + " Vov , thr={thr}, enBin={enBin}, hit={hit}")
                                continue 
    ...
    for key, graph in g_deltaT_corr_vs_bars.items():
        ...
        label="Vov%.2f_th%02d_enBin%02d_%02dHits"%(vov,thr,enBin,hit)

        c=ROOT.TCanvas("c_timeRes_corr_"+label,"",800,600)
        c.SetGrid()
        c.SetLeftMargin(0.15)
        c.SetBottomMargin(0.15)
        graph.SetTitle("")
        ...
        graph.Draw("AP")
        fitRes=ROOT.TF1("fitRes",'pol0',goodBars[vov][0],goodBars[vov][-1])
        ...
        c2=ROOT.TCanvas("c_tRes_spread_"+label,"",800,600)
        c2.SetGrid()
        ...
        latex.DrawLatex(0.15, 0.85,"RMS: %0.4f"%histo.GetRMS())#(in alto a sinistra)
        c2.Update()
        ...
        if(tRes_ave==0 ): continue
           ...
           #print "Vov%.2f th%d enBin%d hits%d: %.1f [ps] spread (RMS) of %.2f%%" % (vov, thr, enBin, hit, tRes_ave, 100*histo.GetRMS())
    ...
    for vov in Vovs:
        for enBin in enBins:
            for hit in Hits:
                ...
                c01=ROOT.TCanvas("c_timeRes_corr_Vov%02f_ALLth_enBin%02d_%02dHits"%(vov,enBin,hit),"",800,600)
                c01.SetGrid()
                ...
                legend.SetFillStyle(0)
                legend.SetTextSize(0.03)
                colors01= [ROOT.kRed, ROOT.kBlue, ROOT.kBlack, ROOT.kGreen, ROOT.kPink, ROOT.kOrange ]
                for i,thr in enumerate(thresholds):
                    ...
                    legend.AddEntry(graph01, "thr %02d"%thr, "ep")
                
                legend.Draw()
                label = "c_timeRes_corr_Vov%02f_ALLth_enBin%02d_%02dHits"%(vov,enBin,hit)
                c01.Update()
                c01.SaveAs(outdir + "/summaryPlots/timeResolution/" + label + ".png")
                c01.SaveAs(outdir + "/summaryPlots/timeResolution/" + label + ".pdf")
            
    #### canvas to plot all Ovs toghter
    if len(Vovs) > 1: 
        for hit in Hits:
            cAll=ROOT.TCanvas("c_timeRes_bestTh_corr_VovAll_enBin%02d_%02dHits"%(1,hit),"",800,600)
            hPad = ROOT.TH2F('hPad_VovAll_enBin%02d_%02dHits'%(1,hit),'', 100, -0.5, 15.5,100, 0,140)
            ...
            for vov in Vovs:
                cAll.cd()
                graph = g_deltaT_bestth_vs_bars[vov, 1, hit]
                graph.SetLineColor(cols[vov])
                graph.SetMarkerColor(cols[vov])

                leg.AddEntry(graph, 'V_{OV} = %.02f V'%vov, 'PL')
                graph.Draw("P same")
                leg.Draw("same")

            cAll.SaveAs(outdir+"/summaryPlots/timeResolution/"+cAll.GetName()+".png")
            cAll.SaveAs(outdir+"/summaryPlots/timeResolution/"+cAll.GetName()+".pdf")
        


    timeRes_thBest_allVov={}
    print("Average time resolution at best th for: ")
    for key,graph in g_deltaT_bestth_vs_bars.items():
        vov, enBin, hit = key
        leg = ROOT.TLegend(0.20, 0.90, 0.60, 0.70)
        c2=ROOT.TCanvas("c_timeRes_bestTh_corr_Vov%.2f_enBin%02d_%02dHits"%(vov,enBin,hit),"",800,600)
        c2.cd()
        hPad1 = ROOT.TH2F('hPad_Vov%.2f_enBin%02d_%02dHits'%(vov,enBin,hit),'', 100, -0.5, 15.5,100, 0,140)
        hPad1.SetTitle("; bar; #sigma_{t} [ps]")
        ...
        fitRes=ROOT.TF1("fitRes",'pol0',goodBars[vov][0],goodBars[vov][-1])
        graph.Fit(fitRes,"QRN")#quite range no-store
        print("Vov %.2f hits %02d Time res: %.01f [ps]"%(vov, hit, fitRes.GetParameter(0)))
        tRes_ave=fitRes.GetParameter(0)
        timeRes_thBest_allVov[vov,enBin,hit]=[fitRes.GetParameter(0),fitRes.GetParError(0)]
        fitRes.SetLineColor(cols[vov])
        fitRes.SetLineStyle(2) # dashed line
        fitRes.Draw("same")
        label = "c_timeRes_bestTh_corr_Vov%.2f_enBin%02d_%02dHits"%(vov,enBin,hit)
        c2.SaveAs(outdir+"/summaryPlots/timeResolution/"+label+".png")
        c2.SaveAs(outdir+"/summaryPlots/timeResolution/"+label+".pdf")
        
        label="Vov%.2f_bestTh_enBin%02d_%02dHits"%(vov,enBin,hit)
        c2=ROOT.TCanvas("c_tRes_spread_"+label,"",800,600)
        c2.SetGrid()
        c2.SetLeftMargin(0.15)
        c2.SetBottomMargin(0.15)
        ...
        if(tRes_ave==0 ): continue
           ...
           if hit == 2:
              print "Vov%.2f enBin%d hits%d: %.1f [ps] spread (RMS) of %.2f%%" % (vov, enBin, hit, tRes_ave, 100*histo.GetRMS())
        #    print(f"Vov{vov} enBin{enBin} hits{hit}: {tRes_ave:.01f} [ps] spread (RMS) of {100*histo.GetRMS():0.2f}%")

    for vov in Vovs:
       for enBin in enBins:
          for hit in Hits:
             index2=g_deltaT_thBest_vs_Vovs[enBin,hit].GetN()
             g_deltaT_thBest_vs_Vovs[enBin,hit].SetPoint(index2,vov,timeRes_thBest_allVov[vov,enBin,hit][0])
             g_deltaT_thBest_vs_Vovs[enBin,hit].SetPointError(index2,0,timeRes_thBest_allVov[vov,enBin,hit][1])


    for key,graph in g_deltaT_thBest_vs_Vovs.items():
        enBin, hit = key
        leg = ROOT.TLegend(0.20, 0.90, 0.60, 0.70)
        c2=ROOT.TCanvas("c_timeRes_bestTh_vs_Vov_enBin%02d_%02dHits"%(enBin,hit),"",800,600)
        c2.cd()
        hPad1 = ROOT.TH2F('hPad_enBin%02d_%02dHits'%(enBin,hit),'', 10, 0.0, 3.5,100, 0,140)
        hPad1.SetTitle("; OV [V]; #sigma_{t} [ps]")
        ...
        c2.SetBottomMargin(0.15)
        graph.SetTitle("")
        graph.GetXaxis().SetTitle("OV [V]")
        graph.GetYaxis().SetTitle("#sigma_{t} [ps]")
        graph.GetXaxis().SetRangeUser(0,3.50)
        graph.GetYaxis().SetRangeUser(0,140)
        graph.SetMarkerStyle(20)
        graph.SetMarkerSize(1.2)
        graph.SetLineWidth(2)
        ...
        graph.Draw("P same")
        label = "c_timeRes_thBest_vs_Vov_enBin%02d_%02dHits"%(enBin,hit)
        if hit == 2 :
           c2.SaveAs(outdir+"/summaryPlots/timeResolution/"+label+".png")
           c2.SaveAs(outdir+"/summaryPlots/timeResolution/"+label+".pdf")
