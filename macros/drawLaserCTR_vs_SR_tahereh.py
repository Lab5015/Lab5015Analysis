...
outFile = ROOT.TFile("/home/petsys/TOFHiR2X/sw_analysis/Lab5015Analysis/plots/drawLaserCTR_vs_SR.root","RECREATE");
...
for run in sorted(runs_dict):
	...
	channels = [220, 222]
        inFile = ROOT.TFile.Open('/home/petsys/TOFHiR2X/sw_analysis/Lab5015Analysis/plots/pulseShape_run%04d_ch1_%d_ch2_%d.root'%(run,channels[0],channels[1]) )
    
        SR = 0.
        thresh = 0.
        channels_names = ['ch1', 'ch2']

        graph1 = inFile.Get('g_ps_totSel_ch1_%d_Vov%.01f'%(channels[0],Vov))
        graph2 = inFile.Get('g_ps_totSel_ch2_%d_Vov%.01f'%(channels[1],Vov))
	...
	fitSR.SetRange( graph1.GetPointX(index_min)-0.001, graph1.GetPointX(index_max)+0.001 )
	fitSR.SetParameters(0,20)
	graph1.Fit(fitSR,'QRS')
	outFile.cd()
	graph1.Write('Run%s_g_ps_totSel_ch1_%d_Vov%.01f'%(run,channels[0],Vov))
	SR1 = fitSR.GetParameter(1)
        thresh1 = int(round(graph1.GetPointY(index_cen)/dac_to_uA[ithMode]))

	fitSR.SetRange( graph2.GetPointX(index_min)-0.001, graph2.GetPointX(index_max)+0.001 )
	fitSR.SetParameters(0,20)
	graph2.Fit(fitSR,'QRS')
	outFile.cd()
	graph2.Write('Run%s_g_ps_totSel_ch2_%d_Vov%.01f'%(run,channels[1],Vov))
	SR2 = fitSR.GetParameter(1)
        thresh2 = int(round(graph2.GetPointY(index_cen)/dac_to_uA[ithMode]))
        ...
        inFile = ROOT.TFile.Open('/home/petsys/TOFHiR2X/sw_analysis/Lab5015Analysis/plots/moduleCharacterization_step2_run%.04d.root'%run)
        
        print('NAME: h1_deltaT_totRatioCorr_bar01L-R_Vov%.02f_th%.02d_energyBin01'%(Vov,thresh))
        histo = inFile.Get('h1_deltaT_totRatioCorr_bar01L-R_Vov%.02f_th%.02d_energyBin01'%(Vov,thresh))
        if histo == None:
            print("MISSING HISTO")
            continue
...
latex.Draw('same')
#c.SaveAs(plotDir+'/CTRvsSR/'+c.GetName()+'.png')
c.Print("c_tRes_vs_SR_bar1.png")
